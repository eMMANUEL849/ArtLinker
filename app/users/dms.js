...
export default function UserDmsScreen() {
  ...
  const initialProviderId = params?.providerId ? String(params.providerId) : "";
  const initialProviderName = params?.providerName
  ...
  useEffect(() => {
    ...
    const unsubscribeSent = onSnapshot(
      ...
      (error) => {
        console.log("SENT DMS ERROR:", error);
        setLoadingChats(false);
      }
    );
    ...
    const unsubscribeReceived = onSnapshot(
      ...
      (error) => {
        console.log("RECEIVED DMS ERROR:", error);
        setLoadingChats(false);
      }
    );
  ...
  useEffect(() => {
    ...
    allUserMessages.forEach((item) => {
      ...
      const otherUserName = isMine
        ? item.receiverName || "Service Provider"
        : item.senderName || "Service Provider";
    ...
    if (initialChatId && initialProviderId && !map.has(initialChatId)) {
      map.set(initialChatId, {
        conversationId: initialChatId,
        chatId: initialChatId,
        jobId: initialJobId || "",
        otherUserId: initialProviderId,
        otherUserName: initialProviderName || "Service Provider",
  ...
  const handleSendMessage = async () => {
    ...
    if (!selectedConversation.otherUserId) {
      Alert.alert("Missing provider", "This conversation has no receiver.");
      return;
    }
    ...
    try {
      ...
      await addDoc(collection(db, "dms"), {
        ...
        receiverName: selectedConversation.otherUserName || "Service Provider",
  ...
  const renderConversation = ({ item }) => {
    ...
    if (isSmallScreen) {
      return (
        <TouchableOpacity
          style={[styles.mobileChatChip, active && styles.mobileChatChipActive]}
          onPress={() => handleSelectConversation(item)}
        >
          <Image
            source={{ uri: item.otherUserAvatar || DEFAULT_AVATAR }}
            style={styles.mobileChatChipAvatar}
          />
          <Text
            style={[styles.mobileChatChipText, active && styles.mobileChatChipTextActive]}
            numberOfLines={1}
          >
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.centerBox}>
          ...
          <Text style={styles.emptyText}>
            You need to be signed in to view your direct messages.
          </Text>
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        >
          ...
          <View style={styles.headerTextWrap}>
            <Text
              style={[
                styles.logo,
                { fontSize: isSmallScreen ? 22 : 28, marginBottom: isSmallScreen ? 4 : 6 },
              ]}
            >
              ArtLinker
            </Text>
            <Text style={[styles.heading, { fontSize: isSmallScreen ? 22 : 28 }]}>
              Direct Messages
            </Text>
            <Text
              style={[
                styles.subheading,
                { fontSize: isSmallScreen ? 12 : 13, lineHeight: isSmallScreen ? 18 : 20 },
              ]}
            >
              View previous messages and send new ones in real time.
            </Text>
          </View>
        </View>
        ...
        {loadingChats && conversations.length === 0 ? (
          ...
          <View style={styles.centerBox}>
            <Ionicons name="chatbubbles-outline" size={46} color="#B8BED0" />
            <Text style={styles.emptyTitle}>No messages yet</Text>
            <Text style={styles.emptyText}>
              When you send or receive a message, it will appear here.
            </Text>
          </View>
        ) : (
          <View
          ...
          >
            {isSmallScreen ? (
              <>
                <View style={styles.mobileChatsBar}>
                  <View style={styles.mobileChatsHeader}>
                  ...
                  <FlatList
                    ...
                    contentContainerStyle={styles.mobileChatsListContent}
                  />
                </View>
...
const styles = StyleSheet.create({
  ...
  mobileChatChipActive: {
    backgroundColor: "#EDE9FE",
    borderColor: "#C4B5FD",
  },
  mobileChatChipAvatar: {
    width: 46,
    height: 46,
    borderRadius: 23,
    marginBottom: 8,
